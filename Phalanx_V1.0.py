        ...
        if z==6:
            if _start2=="N":
                if str(Meuchel_N)==env[OldPosition[0]][OldPosition[1]][1]:
                    break
                print("Kein Meuchelmörder bei Spieler",_start2,"und Einheit",Einheit,"vorhanden!\n")
            if _start2=="S":
    ...
    if z==2:
        ...
        if Einheit[1] not in ["L","B"]:
            if "A" not in Einheit:
                ...
                button1.pack()
                button1.config(height = 5, width = 120)
                button1=tkinter.Button(window,text="Nein",command=lambda arg=2: buttonpress3(arg))
                button1.pack()
                button1.config(height = 5, width = 120)
                window.mainloop()
                if z==1:
